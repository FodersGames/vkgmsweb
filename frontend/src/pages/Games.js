...
const GamesPage = () => {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/api/website/games/public`).then(r => {
    ...
    <div className="bg-[#0a0a0f] text-white min-h-screen" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="pt-16">
        ...
        <div className="py-20 px-6 text-center" style={{ background: 'linear-gradient(180deg, #1a1a2e 0%, #0a0a0f 100%)' }}>
          <h1 className="text-6xl md:text-8xl font-black tracking-tight mb-4" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>OUR GAMES</h1>
          <p className="text-lg text-white/50 max-w-xl mx-auto">Discover the worlds we've built.</p>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-white/40">New games are in development. Stay tuned.</p>
            </div>
          ) : (
              ...
              {games.map((game, idx) => (
                <div key={game.slug} className={`flex flex-col ${idx % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-10 items-center`} data-testid={`game-card-${game.slug}`}>
                  ...
                  <div className="lg:w-1/2">
                    {game.logo_url ? (
                      <img src={game.logo_url.startsWith('/') ? `${API_URL}${game.logo_url}` : game.logo_url}
                        alt={game.name} className="w-full max-w-md mx-auto rounded-xl shadow-2xl shadow-black/50" />
                    ) : game.screenshots?.length > 0 ? (
                      <img src={game.screenshots[0].startsWith('/') ? `${API_URL}${game.screenshots[0]}` : game.screenshots[0]}
                        alt={game.name} className="w-full rounded-xl shadow-2xl shadow-black/50" />
                    ) : (
                      <div className="w-full aspect-video bg-white/5 rounded-xl flex items-center justify-center">
                        <span className="text-white/20 text-2xl font-bold" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>{game.name}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="lg:w-1/2 space-y-6">
                    <h2 className="text-4xl md:text-5xl font-black" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>{game.name}</h2>
                    <p className="text-white/60 leading-relaxed text-lg">{game.description}</p>

                    {/* Screenshots */}
                    {game.screenshots?.length > 1 && (
                      <div className="flex gap-3 overflow-x-auto pb-2">
                        {game.screenshots.slice(1, 4).map((s, i) => (
                          <img key={i} src={s.startsWith('/') ? `${API_URL}${s}` : s}
                            alt={`${game.name} screenshot`} className="h-24 rounded-lg object-cover flex-shrink-0 border border-white/10" />
                        ))}
                      </div>
                    )}
                    ...
                    {game.platforms?.length > 0 && (
                      <div className="flex gap-3 flex-wrap">
                        {game.platforms.map((p, i) => {
                          const platform = PLATFORM_ICONS[p.name];
                          if (!platform) return null;
                          return (
                            <a key={i} href={p.url} target="_blank" rel="noopener noreferrer"
                              className="flex items-center gap-2 px-4 py-2.5 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 hover:border-[#4ECDC4]/30 transition-all group"
                              data-testid={`platform-${p.name}`}>
                              <span className="text-white/60 group-hover:text-[#4ECDC4] transition-colors">{platform.svg}</span>
                              <span className="text-sm font-medium text-white/70 group-hover:text-white transition-colors">{platform.label}</span>
                            </a>
                          );
                        })}
                      </div>
