      ...
      <div className="bg-[#151520] rounded-xl border border-[#2a2a3c] overflow-hidden">
        <div className="px-6 py-4 border-b border-[#2a2a3c] flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div><h3 className="text-base font-semibold text-[#e4e4e7]">Projects / Games</h3><p className="text-xs text-[#71717a]">Manage your API projects</p></div>
          </div>
            ...
            <form onSubmit={handleCreate} data-testid="create-project-form">
              <div className="flex gap-3">
                <div className="flex-1">
                  ...
                  <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-[#0d0d14] border border-[#2a2a3c] text-[#e4e4e7] rounded-lg text-sm px-3 py-2.5 focus:border-[#4ECDC4] focus:outline-none" required data-testid="project-name-input" />
                </div>
        ...
        <div className="p-6">
          ...
          {projects.length === 0 ? (
            ...
            <div className="space-y-3" data-testid="projects-list">
              {projects.map(p => (
                <div key={p.slug} className="bg-[#1c1c2e] border border-[#2a2a3c] rounded-xl p-4 hover:border-[#4ECDC4]/20 transition-all" data-testid={`project-card-${p.slug}`}>
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-3">
                      ...
                      <div><h4 className="font-medium text-[#e4e4e7]">{p.name}</h4><p className="text-xs text-[#71717a] font-mono">slug: {p.slug}</p></div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-[#71717a]">by {p.created_by}</span>
                      {hasPermission('delete_projects') && (
                        <button onClick={() => handleDelete(p.slug, p.name)} className="p-2 border border-[#2a2a3c] hover:border-red-500/30 rounded-lg text-[#71717a] hover:text-red-400 transition-all" data-testid={`delete-project-${p.slug}`}><Trash2 size={14} /></button>
                      )}
                    </div>
                  </div>
                </div>
