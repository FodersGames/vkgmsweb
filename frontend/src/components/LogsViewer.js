      ...
      <div className="bg-[#151520] rounded-xl border border-[#2a2a3c] overflow-hidden">
        <div className="px-6 py-4 border-b border-[#2a2a3c]">
          <div className="flex items-center gap-3">
            ...
            <div><h3 className="text-base font-semibold text-[#e4e4e7]">Activity Logs</h3><p className="text-xs text-[#71717a]">View project activity</p></div>
          </div>
        </div>
        <div className="p-6 bg-[#1c1c2e] border-b border-[#2a2a3c]">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              ...
              <select value={filters.type} onChange={e => setFilters({...filters, type: e.target.value})} className="w-full bg-[#0d0d14] border border-[#2a2a3c] text-[#e4e4e7] rounded-lg text-sm px-3 py-2.5 focus:border-[#4ECDC4] focus:outline-none" data-testid="filter-type">
            ...
            <div><label className="block text-xs font-semibold text-[#71717a] mb-2">User</label><input type="text" value={filters.user} onChange={e => setFilters({...filters, user: e.target.value})} className="w-full bg-[#0d0d14] border border-[#2a2a3c] text-[#e4e4e7] rounded-lg text-sm px-3 py-2.5 focus:border-[#4ECDC4] focus:outline-none" data-testid="filter-user" /></div>
            <div><label className="block text-xs font-semibold text-[#71717a] mb-2">UID</label><input type="text" value={filters.uid} onChange={e => setFilters({...filters, uid: e.target.value})} className="w-full bg-[#0d0d14] border border-[#2a2a3c] text-[#e4e4e7] rounded-lg text-sm px-3 py-2.5 focus:border-[#4ECDC4] focus:outline-none" data-testid="filter-uid" /></div>
            <div><label className="block text-xs font-semibold text-[#71717a] mb-2">Limit</label><input type="number" min="10" max="1000" value={filters.limit} onChange={e => setFilters({...filters, limit: parseInt(e.target.value)})} className="w-full bg-[#0d0d14] border border-[#2a2a3c] text-[#e4e4e7] rounded-lg text-sm px-3 py-2.5 focus:border-[#4ECDC4] focus:outline-none" data-testid="filter-limit" /></div>
          </div>
        ...
        <div className="p-6">
          ...
          <div className="overflow-x-auto rounded-lg border border-[#2a2a3c]">
            <table className="w-full" data-testid="logs-table">
              <thead><tr className="bg-[#1c1c2e]">
                <th className="text-left p-3 text-xs font-semibold text-[#71717a] border-b border-[#2a2a3c]">Type</th>
                <th className="text-left p-3 text-xs font-semibold text-[#71717a] border-b border-[#2a2a3c]">Time</th>
                <th className="text-left p-3 text-xs font-semibold text-[#71717a] border-b border-[#2a2a3c]">User</th>
                <th className="text-left p-3 text-xs font-semibold text-[#71717a] border-b border-[#2a2a3c]">Message</th>
              </tr></thead>
              <tbody>
                {logs.map((l, i) => (
                  <tr key={i} className="hover:bg-[#1c1c2e] border-b border-[#2a2a3c] last:border-0">
                    ...
                    <td className="p-3 text-sm text-[#e4e4e7]">{l.user || '-'}</td>
                    <td className="p-3 text-sm text-[#e4e4e7]">{l.message}</td>
                  </tr>
                ))}
              </tbody>
            </table>
