      ...
      <div className="bg-[#151520] rounded-xl border border-[#2a2a3c] overflow-hidden">
        <div className="px-6 py-4 border-b border-[#2a2a3c] flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div><h3 className="text-base font-semibold text-[#e4e4e7]">Variables</h3><p className="text-xs text-[#71717a]">Manage project variables</p></div>
          </div>
            ...
            <form onSubmit={handleCreate} data-testid="create-variable-form">
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={form.variable_name} onChange={e => setForm({ ...form, variable_name: e.target.value })} className="w-full bg-[#0d0d14] border border-[#2a2a3c] text-[#e4e4e7] rounded-lg text-sm px-3 py-2.5 focus:border-[#4ECDC4] focus:outline-none" required data-testid="variable-name-input" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#71717a] mb-2 uppercase tracking-wider">Values</label>
                  {form.values.map((v, i) => (
                    <div key={i} className="flex gap-2 mb-2">
                      <input type="text" value={v} onChange={e => setForm(p => ({ ...p, values: p.values.map((x, j) => j === i ? e.target.value : x) }))} className="flex-1 bg-[#0d0d14] border border-[#2a2a3c] text-[#e4e4e7] rounded-lg text-sm px-3 py-2.5 focus:border-[#4ECDC4] focus:outline-none" data-testid={`value-input-${i}`} />
                      {form.values.length > 1 && <button type="button" onClick={() => setForm(p => ({ ...p, values: p.values.filter((_, j) => j !== i) }))} className="px-3 py-2 border border-[#2a2a3c] hover:border-red-500/30 rounded-lg text-red-400"><Trash2 size={14} /></button>}
                    </div>
                  ))}
          ...
          {variables.length === 0 ? <div className="text-center py-12 text-[#71717a]">No variables yet.</div> : (
            <div className="space-y-3" data-testid="variables-list">
              {variables.map(v => {
                ...
                return (
                  <div key={v.variable_name} className="bg-[#1c1c2e] border border-[#2a2a3c] rounded-xl p-4 hover:border-[#4ECDC4]/20 transition-all">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      {!isEditing && (
                        <div className="flex gap-2">
                          {hasPermission('edit_variables') && <button onClick={() => setEditingVar({...v})} className="p-2 border border-[#2a2a3c] hover:border-[#4ECDC4]/30 rounded-lg text-[#71717a] hover:text-[#4ECDC4]" data-testid={`edit-variable-${v.variable_name}`}><Edit2 size={14} /></button>}
                          {hasPermission('delete_variables') && <button onClick={() => handleDelete(v.variable_name)} className="p-2 border border-[#2a2a3c] hover:border-red-500/30 rounded-lg text-[#71717a] hover:text-red-400" data-testid={`delete-variable-${v.variable_name}`}><Trash2 size={14} /></button>}
                        </div>
                      )}
                    </div>
                    {isEditing ? (
                      <div className="space-y-2">
                        {editingVar.values.map((val, i) => (
                          <div key={i} className="flex gap-2">
                            <input type="text" value={val} onChange={e => setEditingVar(p => ({...p, values: p.values.map((x, j) => j === i ? e.target.value : x)}))} className="flex-1 bg-[#0d0d14] border border-[#2a2a3c] text-[#e4e4e7] rounded-lg text-sm px-3 py-2 focus:border-[#4ECDC4] focus:outline-none" />
                            {editingVar.values.length > 1 && <button onClick={() => setEditingVar(p => ({...p, values: p.values.filter((_, j) => j !== i)}))} className="px-3 py-2 border border-[#2a2a3c] rounded-lg text-red-400"><Trash2 size={14} /></button>}
                          </div>
                        ))}
                        ...
                        <div className="flex gap-2 mt-3">
                          ...
                          <button onClick={() => setEditingVar(null)} className="bg-[#1c1c2e] text-[#e4e4e7] border border-[#2a2a3c] rounded-lg px-4 py-2 text-sm flex items-center gap-2"><X size={14} />Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <div className="flex flex-wrap gap-2">
                        {v.values.map((val, i) => <span key={i} className="px-3 py-1.5 bg-[#0d0d14] border border-[#2a2a3c] rounded-lg text-sm text-[#e4e4e7] font-mono">{val}</span>)}
                      </div>
                    )}
                  </div>
                );
