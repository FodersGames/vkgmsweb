...
export const AccountSettings = () => {
  ...
  return (
    <div className="max-w-lg">
      <h2 className="text-2xl font-black text-gray-900 mb-1" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>MY ACCOUNT</h2>
      <p className="text-xs text-gray-400 mb-8">Update your display name and username.</p>
      ...
      <div className="bg-white border border-gray-200 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-900 mb-1.5">Username</label>
            <input
              type="text"
              required
              minLength={3}
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              type="submit"
              disabled={saving}
              className="flex items-center gap-2 bg-gray-900 hover:bg-gray-800 text-white px-5 py-2.5 text-sm font-semibold transition-colors disabled:opacity-50"
            >
      ...
      {canAdjustLoyalty && (
        <div className="bg-white border border-gray-200 p-6 mt-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-sm font-bold text-gray-900">Loyalty Balance Adjustment</h3>
          </div>
          <p className="text-xs text-gray-400 mb-4">Add or subtract loyalty spend ($) from your own account. Tier recalculates automatically.</p>

          <form onSubmit={handleLoyalty} className="space-y-3">
            <div>
              <label className="block text-xs font-semibold text-gray-900 mb-1.5">Amount ($)</label>
              <input
                type="number"
                step="0.01"
                required
              ...
              />
              <p className="text-[10px] text-gray-400 mt-0.5">Positive to add, negative to remove. Tiers: Bronze $0 · Silver $25 · Gold $100 · Diamond $250</p>
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-900 mb-1.5">Reason (optional)</label>
              <input
                type="text"
                maxLength={100}
                value={loyaltyReason}
