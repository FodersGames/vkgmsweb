      ...
      <Card className="overflow-hidden">
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-[#1C1917] dark:text-[#e4e4e7]">Games Management</h3>
            ...
            <form onSubmit={handleSubmit} data-testid="game-form">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    {form.logo_url && <img src={form.logo_url.startsWith('/') ? `${API_URL}${form.logo_url}` : form.logo_url} alt="logo" className="w-14 h-14 object-cover border border-[#E8E3DB] dark:border-[#2a2a3c]" />}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {form.screenshots.map((s, i) => (
                      <div key={i} className="relative group">
                        <img src={s.startsWith('/') ? `${API_URL}${s}` : s} alt="" className="w-20 h-14 object-cover border border-[#E8E3DB] dark:border-[#2a2a3c]" />
                    ...
                    <label className="cursor-pointer w-20 h-14 bg-[#F0EDE8] dark:bg-[#0d0d14] border border-dashed border-[#E8E3DB] dark:border-[#2a2a3c] hover:border-[#4ECDC4]/50 flex items-center justify-center transition-colors">
                    ...
                    {PLATFORMS.map(p => {
                      ...
                      return (
                        <div key={p.id} className="flex items-center gap-3">
                          <label className={`flex items-center gap-2 px-3 h-9 border cursor-pointer transition-colors text-sm shrink-0 ${active ? 'border-[#4ECDC4] bg-[#4ECDC4]/10 text-[#4ECDC4]' : 'border-[#E8E3DB] dark:border-[#2a2a3c] text-[#71717a]'}`}>
                ...
                <div>
                  ...
                  <div className="relative max-w-[180px]">
                    ...
                    <input
                      ...
                      className="w-full pl-7 pr-3 py-2 border border-[#E8E3DB] dark:border-[#2a2a3c] text-sm text-[#1C1917] dark:text-[#e4e4e7] bg-white dark:bg-[#111118] focus:outline-none focus:border-[#4ECDC4]"
                    />
                  </div>
                  <p className="text-[10px] text-[#71717a] mt-1">Leave empty or 0 for a free game. Price used for Stripe checkout.</p>
                </div>

                <label className="flex items-center gap-3 px-4 py-3 bg-[#F0EDE8] dark:bg-[#0d0d14] border border-[#E8E3DB] dark:border-[#2a2a3c] cursor-pointer hover:border-[#4ECDC4]/30 transition-colors" data-testid="featured-toggle">
                  ...
                  <div>
                    <span className="text-sm text-[#1C1917] dark:text-[#e4e4e7] font-medium">Featured Game</span>
        ...
        <CardBody>
          ...
          {games.length === 0 ? (
            ...
            <div className="space-y-2" data-testid="games-list">
              {games.map(g => (
                <div key={g.slug} className="flex items-center gap-4 p-4 bg-[#F9F7F4] dark:bg-[#111118] border border-[#E8E3DB] dark:border-[#2a2a3c] hover:border-[#4ECDC4]/20 transition-colors">
                  {g.logo_url
                    ? <img src={g.logo_url.startsWith('/') ? `${API_URL}${g.logo_url}` : g.logo_url} alt="" className="w-12 h-12 object-cover border border-[#E8E3DB] dark:border-[#2a2a3c] shrink-0" />
                    : <div className="w-12 h-12 bg-[#F0EDE8] dark:bg-[#0d0d14] border border-[#E8E3DB] dark:border-[#2a2a3c] flex items-center justify-center shrink-0"><Gamepad2 size={18} className="text-[#71717a]" /></div>}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h4 className="text-sm font-semibold text-[#1C1917] dark:text-[#e4e4e7]">{g.name}</h4>
                    ...
                    {g.platforms?.length > 0 && (
                      <div className="flex gap-1 mt-1.5 flex-wrap">
                        {g.platforms.map((p, i) => <span key={i} className="text-[10px] px-1.5 py-0.5 bg-[#F0EDE8] dark:bg-[#0d0d14] border border-[#E8E3DB] dark:border-[#2a2a3c] rounded text-[#71717a]">{p.name}</span>)}
                      </div>
                    )}
                  </div>
